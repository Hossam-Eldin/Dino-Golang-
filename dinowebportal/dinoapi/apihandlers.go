...
func (handler *DinoRESTAPIHandler) searchHandler(w http.ResponseWriter, r *http.Request)  {
	...
	if !ok{
		...
		fmt.Fprintln(w, `No search criteria found, you can either search by nickname via /api/dinos/nickname/rex , or
...
func (handler *DinoRESTAPIHandler) editsHandler(w http.ResponseWriter, r *http.Request)  {
	...
	if err !=nil {
		...
		fmt.Fprintln(w, `Erro occured while processing request %v`, err)
	}
